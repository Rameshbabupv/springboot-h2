...
public class DivisionRequest {
    ...
    @Size(min = 2, max = 50, message = "Division name must be between 2 and 50 characters")
    @Pattern(regexp = "^[a-zA-Z0-9\\s&\\-/]+$", message = "Division name can only contain letters, numbers, spaces, &, -, and /")
    private String name;

    @NotBlank(message = "Division code is required")
    @Size(min = 2, max = 10, message = "Division code must be between 2 and 10 characters")
    @Pattern(regexp = "^[A-Z0-9\\-_]+$", message = "Division code can only contain uppercase letters, numbers, -, and _")
    private String code;
